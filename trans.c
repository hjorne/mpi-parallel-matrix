...
double** CalculateTranspose(int num_rows, int row_length, int num_threads, double** orig_matrix)
{
    ...
    double** transpose = (double**) malloc(row_length * sizeof(
                                               double*)); //Init first dimension of transpose

    for(i = 0; i < row_length; i++) {
        transpose[i] = malloc(num_rows * sizeof(
                                  double)); //This must be done before the creation of the threads since each thread requires the every row of the transpose
    }

    for(i = 0; i < (num_threads - 1);
            i++) { //Once again adjust by 1 to allow for reuse of parent thread
        struct copyThreadArgs* new_args = malloc(sizeof(struct
                                                        copyThreadArgs)); //Create arg struct for each thread, might want to free this at some point, not sure where
